...
import { createDashboardAction } from '@/actions/admin/powerbi/create-dashboard'
		...
		try {
			...
			if (result.error) {
				toast.error('Error al crear dashboard', {
					description: result.error || 'No se pudo crear el dashboard',
				})
				throw new Error(result.error || 'Error al crear dashboard')
			}
			...
			if (
				!(
					...
					error.message.includes('Error al crear dashboard')
				)
			) {
