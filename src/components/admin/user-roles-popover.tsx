...
export function UserRolesPopover({ user, onRolesUpdated }: UserRolesPopoverProps) {
  ...
  const handleAddRole = async (roleId: string) => {
    ...
    try {
      ...
      if (!(error instanceof Error && error.message.includes("Error al agregar rol"))) {
  ...
  const handleRemoveRole = async (roleId: string) => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/admin/users/roles/${user.id}/${roleId}`, {
        method: "DELETE",
      });
      ...
      // Solo mostrar toast si no se mostró antes
      if (!(error instanceof Error && error.message.includes("Error al eliminar rol"))) {
        toast.error("Error inesperado", {
