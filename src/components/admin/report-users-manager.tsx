...
import { updateDashboardUsersAction } from '@/actions/admin/powerbi/update-dashboard-users'
...
export function ReportUsersManager({
	reportId,
	reportTitle,
}: ReportUsersManagerProps) {
	...
	const loadData = async () => {
		try {
			...
			const filteredUsers = users.filter(
				(u: any) => u.active && !u.role.includes('admin'),
			)
			setAllUsers(filteredUsers)

			// Cargar usuarios con acceso al reporte
			const reportUsersResponse = await fetch(
				`/api/admin/powerbi/${reportId}/users`,
			)
	...
	const handleSave = async () => {
		try {
			...
			const result = await updateDashboardUsersAction(reportId, selectedUserIds)

			if (result.error) throw new Error(result.error || 'Error al guardar')
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Selecciona los usuarios que pueden ver este reporte. Los
					administradores tienen acceso a todos los reportes.
				</CardDescription>
			</CardHeader>
